...
import static com.ray3k.template.Resources.SpineGameOverHand.*;

public class GameOver1Screen extends JamScreen {
    private Stage stage;
    ...
    public void show() {
        ...
        label = new Label("Click to try again", skin);
        ...
        stage.addListener(new InputListener() {
            ...
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                ...
                if (state.getCurrent(0).getAnimation() == animationStand) {
                    state.setAnimation(0, animationGoToPosition, false);
                    state.addAnimation(0, animationPosition, false, 0);
                } else if (pain < 1){
                    label.setText("Keep clicking! FASTER!");
                    state.setAnimation(1, animationSmack, false);
                    pain += .10f;
                }
    ...
    public void act(float delta) {
        ...
        if (current0 != animationStand && current0 != animationGoToPosition) {
            pain -= .35f * delta;
            Animation animation;
            var loop = true;
            if (pain < par) {
            ...
            if (pain >= 1f) {
                animation = animationHurt04;
                loop = false;
                par = 1f;
            } else if (pain >= .75f) {
                animation = animationHurt03;
                par = .75f;
            } else if (pain >= .5f) {
                animation = animationHurt02;
                par = .5f;
            } else if (pain >= .25f) {
                animation = animationHurt01;
                par = .25f;
            } else if (pain > 0f){
                animation = animationHurt00;
                par = 0;
            } else {
